...
public class CharacterController {
    ...
    public ResponseEntity<Character> save(@RequestParam("token") String token, @RequestParam("username") String username, @RequestBody Character entity) {
        ...
        if (exists)
            logRepository.save(new Log(UUID.randomUUID(), Action.CREATE_CHARACTER, username, LocalDateTime.now(), "character", result.getId()));
        else
            logRepository.save(new Log(UUID.randomUUID(), Action.EDIT_CHARACTER, username, LocalDateTime.now(), "character", result.getId()));
    ...
    public ResponseEntity<String> delete(@RequestParam("token") String token, @RequestParam("username") String username, @PathVariable("id") UUID id) {
        ...
        logRepository.save(new Log(UUID.randomUUID(), Action.DELETE_CHARACTER, username, LocalDateTime.now(), "character", id));
